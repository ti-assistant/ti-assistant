...
import { Handler, getOppositeHandler } from "../../../src/util/api/data";
...
import { buildFactions } from "../../../src/data/GameData";
...
export default async function handler(
  ...
  try {
    ...
    while (shouldRetry && numAttempts > 0) {
      shouldRetry = await updateInTransaction(db, gameRef, req, res, gameTime);
      if (shouldRetry) {
  ...
  actionLog.forEach((storedLogEntry) => {
    ...
    switch (action) {
      ...
      case "REPLACE": {
        ...
        console.log("[REPLACING ENTRY]", logEntry);
        t.update(
